...
**Worktree:** `ERP_Avgust_is_petya_e19_1` (создан от `origin/main` @ `8f69bea`).
**Приоритет:** 🟢 feature E19. **Старт: 2026-04-26 по go PO.**
**Срок:** ~1 день.

> **ВАЖНО — независимость от E18.** В мастер-спеке E19-1 упоминается dependency `get_llm_provider` (E18 headers). Однако E18 ещё **не запущен** — мы стартуем E19 первым из-за срочности (87-страничный PDF висит без async). Поэтому в этом ТЗ:
> - **Используй singleton `OpenAIVisionProvider` из текущего sync endpoint'а** — он создаётся через `get_provider_from_settings()` или аналогично (см. `recognition/app/api/parse.py` как сделан sync endpoint).
> - **`get_llm_provider` dependency пока создавать НЕ нужно** — это будет сделано в E18-1 позже. Когда E18-1 замержат, async endpoint автоматически начнёт получать override headers.
> - Headers `X-LLM-*` пока **игнорируем** в async endpoint. Если они будут переданы — просто не читаем их (текущий sync endpoint тоже их не читает).
...
Используй **singleton provider** как и текущий sync endpoint (`get_provider()` или аналог — посмотри как сделан `/v1/parse/spec`):
...
async def parse_spec_async(
    ...
    provider = get_singleton_provider()  # как в sync endpoint
...
2. ✅ В фоне отправляются callbacks: `started`, `page_done` (по странице), `finished` (с llm_costs если уже есть в SpecParseResponse, иначе пустое) ИЛИ `failed` ИЛИ `cancelled`.
...
6. ⏳ E18 интеграция (X-LLM-* headers) — отложено до E18-1, см. ВАЖНО блок в начале ТЗ.
7. ✅ Все тесты зелёные. Live-прогон 3 голд-PDF через async с моком callback'а возвращает те же items что и sync endpoint (153/29/199).
...
Сейчас sync endpoint держит backend 10-15 мин (на 87-страничном PDF
зависает совсем) — нужен async pattern.
...
срочности. В ТЗ есть отдельный блок «ВАЖНО — независимость от E18» —
прочитай первым делом. Используй singleton provider из текущего sync
endpoint, X-LLM-* headers пока игнорируй.

ПАРАЛЛЕЛЬНО работает другая команда (AC Rating) на ветках
ac-rating/* — НЕ заходи в backend/ac_*, frontend/app/ratings/,
ac-rating/. Если планируешь править shared файлы (settings.py,
urls.py, docker-compose.yml) — пинг Андрею ДО коммита.
