...
6. **Модель + цена ненавязчиво везде.** PO попросил везде где упоминается job показывать **какая модель распознавала и сколько стоило**, мелким серым шрифтом (не выделяется). См. master spec E18 раздел «Модель + цена — обязательная сквозная мета-информация» — список touchpoint'ов и формат. Применяется к **всем** компонентам E19: toast завершения, popover jobs panel (recent row), banner в смете, мета-блок самой сметы.

## Production model decision (PO 2026-04-25)

После захода 3/10 QA: **DeepSeek V4-Pro thinking high** даёт 100% точность на всех 3 голд-PDF (vs gpt-5.4 ±1 variance). Цена в 4× ниже. **Платим временем** (4-15 мин на PDF против 1-3 мин у gpt-5.4) — лечим через E19 background jobs.

Default LLM-профиль для production: «DeepSeek V4-Pro thinking high». PO выбирает из dropdown в `PdfImportDialog`, может переключиться на gpt-5.4 (быстрый default) если важна скорость.
