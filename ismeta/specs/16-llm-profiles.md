...
### «Модель + цена» — обязательная сквозная мета-информация (PO 2026-04-25)

PO попросил во всех UI-touchpoint распознавания показывать **какая модель и сколько стоило**, ненавязчиво (мелким шрифтом, серым, не привлекая внимание):

1. **PdfImportDialog результат** — блок alert после import (уже описано выше): «199 позиций · DeepSeek v4-pro · $0.12».
2. **Banner в смете** (после async impart по E19) — после `done`: «✓ Распознано 199 позиций · DeepSeek v4-pro · $0.12 · 8 мин».
3. **Toast при завершении** (E19) — «199 позиций ($0.12, DeepSeek v4-pro)».
4. **Popover jobs panel** (E19) — на recent-row: «✓ Объект А · 199 поз · $0.12 · DeepSeek v4-pro · 8 мин».
5. **Сама смета — мета-блок** (постоянное место): малый серый текст где-то у заголовка сметы или в sticky-note (UI-12): «Распознано: DeepSeek v4-pro · $0.12 · 25 апр». Не выделяется.
6. **История ImportLog** (отдельная страница / эндпоинт для будущего) — table с колонками: дата, файл, модель, цена, items, duration.

Источник данных: `ImportLog.profile.name + ImportLog.cost_usd + ImportLog.llm_metadata.extract.model`.
