...
Я уже извлёк строки таблицы по bbox из text-layer PDF. Данные передаются как
**markdown-таблица**: каждая строка таблицы соответствует одной row из PDF, колонки
явно разделены `|`. Колонки: row_index | pos | name | model | brand | manufacturer | unit | qty | mass | comments | section_heading | raw_blocks.

ВАЖНО про markdown-table формат:
- Каждая `|`-разделённая колонка — это отдельная cell. Не путай столбцы между собой.
- Пустая cell отображается как пробел между `|` (например `| | name | | | ...`).
- Это надёжнее JSON: каждая row — одна линия таблицы, колонки фиксированы по позиции.
- row_index — индекс bbox-row в исходном PDF (используй для source_row_index в output).
- raw_blocks — резерв на случай пустых cells (исходные текст-блоки строки).
...
def _row_to_dict(row: TableRow) -> dict:
    ...
    Используется legacy text+vision multimodal path. Variant B (markdown table)
    для основного text path — см. `_rows_to_markdown_table`.
    """
    return {
...
def _escape_md_cell(s: str) -> str:
    """Экранирование cell для markdown-таблицы — `|` ломает таблицу."""
    if not s:
        return ""
    return s.replace("|", "/").replace("\n", " ").strip()


def _rows_to_markdown_table(rows: list[TableRow]) -> str:
    """TD-11B: сериализация rows в markdown-таблицу для LLM.

    Формат: одна строка-заголовок + одна строка-разделитель + по строке на row.
    Колонки фиксированы: row_index | pos | name | model | brand | manufacturer
                         | unit | qty | mass | comments | section_heading | raw_blocks.

    Markdown-таблица читается LLM значительно надёжнее JSON со вложенным cells:
    - каждая row — одна линия (no nesting)
    - колонки явно разделены `|`
    - LLM видит структуру таблицы напрямую, без интерпретации dict ключей

    Гипотеза TD-11B: Class L cascade (qty/model переезжает между соседними rows)
    меньше проявляется на табличной форме чем на JSON dict.
    """
    columns = [
        "row_index", "pos", "name", "model", "brand", "manufacturer",
        "unit", "qty", "mass", "comments", "section_heading", "raw_blocks",
    ]
    lines = ["| " + " | ".join(columns) + " |"]
    ...
    for r in rows:
        cells = r.cells or {}
        raw_blocks_str = " ;; ".join(_escape_md_cell(b) for b in (r.raw_blocks or []))
        row_cells = [
            str(r.row_index),
            _escape_md_cell(cells.get("pos", "")),
            _escape_md_cell(cells.get("name", "")),
            _escape_md_cell(cells.get("model", "")),
            _escape_md_cell(cells.get("brand", "")),
            _escape_md_cell(cells.get("manufacturer", "")),
            _escape_md_cell(cells.get("unit", "")),
            _escape_md_cell(cells.get("qty", "")),
            _escape_md_cell(cells.get("mass", "")),
            _escape_md_cell(cells.get("comments", "")),
            "true" if r.is_section_heading else "",
            raw_blocks_str[:300],  # truncate чтобы prompt не раздувался
        ]
...
async def normalize_via_llm(
    ...
    *,
    ...
    # TD-11B: markdown-таблица вместо JSON для лучшей читаемости LLM.
    rows_payload = _rows_to_markdown_table(filtered_rows)
    user_input = _build_user_input(current_section, sticky_parent_name, rows_payload)
