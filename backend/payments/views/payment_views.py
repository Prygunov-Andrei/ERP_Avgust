...
class PaymentRegistryViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        from payments.services.registry_service import RegistryService
        payment_request = self.get_object()
        RegistryService.approve(payment_request, request.user)
    ...
    def pay(self, request, pk=None):
        """Переводит заявку в статус 'Оплачено'"""
        from payments.services.registry_service import RegistryService
        payment_request = self.get_object()
        RegistryService.pay(payment_request)
        serializer = self.get_serializer(payment_request)
        return Response(serializer.data)
    ...
    def cancel(self, request, pk=None):
        ...
        from payments.services.registry_service import RegistryService
        payment_request = self.get_object()
        RegistryService.cancel(payment_request)
        serializer = self.get_serializer(payment_request)
        return Response(serializer.data)
    ...
    def get_queryset(self):
        ...
        if self.action == 'list':
            queryset = queryset.annotate(
                _annotated_balance=Coalesce(Sum('credit_entries__amount'), Decimal('0'))
                - Coalesce(Sum('debit_entries__amount'), Decimal('0')),
            )
        return queryset
