...
## 2.6 Массовый импорт счетов

Для загрузки нескольких счетов одновременно используйте функцию массового импорта.

### Как загрузить

1. Перейдите на вкладку **Счета на оплату**
2. Нажмите кнопку **«Массовый импорт»** (рядом с «Создать счёт»)
3. Выберите файлы одним из способов:
   - Нажмите **«Выбрать файлы»** и выберите нужные файлы
   - **Перетащите** файлы в область загрузки
4. Поддерживаемые форматы: **PDF**, **Excel** (XLSX, XLS), **изображения** (PNG, JPG)
5. Максимум 50 файлов за одну сессию
6. Нажмите **«Начать обработку»**

### Что происходит при обработке

- Каждый файл распознаётся через искусственный интеллект (LLM)
- Автоматически определяются: номер счёта, дата, поставщик, позиции, суммы
- Создаётся карточка счёта в статусе **«На проверке»**
- Если поставщик новый — автоматически создаётся контрагент
- Позиции из счёта сохраняются, но товары **не добавляются в каталог** до проверки оператором
- Если обнаружен дубликат (тот же номер + сумма + ИНН) — счёт создаётся с предупреждением

### Прогресс и результаты

- Во время обработки отображается прогресс-бар с количеством обработанных файлов
- По завершении — сводка: успешно / с ошибками / дубликаты
- Список созданных счетов — нажмите на любой для перехода к детальному просмотру
- Счета с ошибками распознавания тоже создаются — их можно заполнить вручную

### Проверка импортированных счетов

Все импортированные счета появляются в общем списке с источником «Массовый импорт».

**Порядок проверки:**

1. Откройте счёт — слева отображается PDF/скан документа, справа — форма для редактирования
2. Проверьте и при необходимости исправьте: контрагента, суммы, категорию расходов, объект
3. Нажмите **«Подтвердить»** — статус сменится на **«Проверен»**, товары из позиций будут добавлены в каталог
4. *(Опционально)* Нажмите **«В реестр»** — счёт попадёт в Реестр оплат для согласования

> **Примечание:** Не все счета нужно отправлять в реестр. Счета в статусе «Проверен» сохраняются для сравнения цен и использования в сметах.

### Статусы счетов

| Статус | Описание |
|---|---|
| Распознаётся | Файл обрабатывается искусственным интеллектом |
| На проверке | Оператор проверяет и исправляет распознанные данные |
| Проверен | Данные подтверждены, товары добавлены в каталог. Счёт доступен для сравнения цен |
| В реестре | Отправлен на оплату, ожидает решения контролёра |
| Согласовано | Контролёр одобрил, формируется платёжное поручение |
| Отправляется | Платёжное поручение отправлено в банк |
| Оплачен | Оплата подтверждена банком |
| Отменён | Отменён на любом этапе |

### Работа с разными ролями

- **Сметчик** — загружает счета поставщиков **из редактора сметы** (вкладка «Счета поставщиков»). Такие счета привязаны к смете и не отображаются в данном разделе. Подробнее — в разделе «Коммерческие предложения → Инструкции → Счета поставщиков в процессе расчёта».
- **Снабженец** — загружает и обрабатывает счета на оплату в данном разделе. Видит только обычные счета (не привязанные к сметам). Может сравнить цены и отправить выбранный счёт в реестр.

> **Примечание:** Список счетов в разделе «Финансы → Счета на оплату» показывает только обычные (не исследовательские) счета. Счета, загруженные сметчиком из редактора сметы для ценового исследования, видны только на вкладке «Счета поставщиков» соответствующей сметы.
...
1. **Оператор** создаёт/загружает счёт → статус **«Распознаётся»** → затем **«На проверке»**
2. **Оператор** проверяет данные и нажимает **«Подтвердить»** → статус **«Проверен»**, товары добавлены в каталог
3. **Оператор** решает отправить в реестр → нажимает **«В реестр»** → статус **«В реестре»**
4. **Контролёр** просматривает и нажимает **«Согласовать»**
   - В истории сохраняется: кто и когда согласовал
   - Для безналичного платежа формируется Платёжное поручение (черновик в банке)
5. Банк подтверждает оплату → статус **«Оплачено»**
