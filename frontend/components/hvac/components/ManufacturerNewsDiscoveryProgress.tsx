...
export default function ManufacturerNewsDiscoveryProgress({ onComplete, onError }: ManufacturerNewsDiscoveryProgressProps) {
  ...
  const pollStatus = async () => {
    try {
      ...
      const errorMsg = axios.isAxiosError(err) ? (err.response?.data as Record<string, string>)?.message || 'Не удалось получить статус поиска по производителям' : 'Не удалось получить статус поиска по производителям';
      setError(errorMsg);
      if (onError) {
        onError(errorMsg);
      }
