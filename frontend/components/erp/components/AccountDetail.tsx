      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-card rounded-xl border border-border p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <div className="text-sm text-muted-foreground mb-2">Валюта</div>
              <div className="font-medium text-foreground">{account.currency}</div>
            </div>

            {account.bank_name && (
              <div>
                <div className="text-sm text-muted-foreground mb-2">Банк</div>
                <div className="font-medium text-foreground">{account.bank_name}</div>
              </div>
            )}

            {(account.bic || account.bik) && (
              <div>
                <div className="text-sm text-muted-foreground mb-2">БИК</div>
            ...
            {account.location && (
              <div>
                <div className="text-sm text-muted-foreground mb-2">Местоположение</div>
                <div className="font-medium text-foreground">{account.location}</div>
              </div>
            )}
          </div>
          ...
          {account.description && (
            <div className="mt-6 pt-6 border-t border-border">
              ...
              <div className="text-foreground">{account.description}</div>
            </div>
          )}
        </div>
          ...
          <TabsContent value="balances" className="mt-6">
            ...
            {balancesLoading ? (
              ...
              <div className="bg-muted border-2 border-dashed border-border rounded-xl p-12 text-center">
                ...
                <p className="text-muted-foreground">Нет данных об остатках</p>
              </div>
            ) : (
                    ...
                    <tbody className="divide-y divide-border">
                      {balancesArray
                        ...
                        .map((balance: AccountBalance) => (
                          <tr key={balance.id} className="hover:bg-muted transition-colors">
