  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <>
          <div className="px-3 py-1.5 text-xs text-muted-foreground border-b bg-muted flex items-center gap-1">
            {data.source === 'local' ? (
          ...
          {data.results.map((result, idx) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {result.is_local && (
                  <span className="shrink-0 px-1.5 py-0.5 text-[10px] font-medium bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded">
                    В базе
                  </span>
                )}
              </div>
...
export function FNSCheckTabContent({ data }: { data: FNSQuickCheckResponse }) {
  ...
  const riskColor = summary.risk_level === 'low' ? 'text-green-600 bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800'
    : summary.risk_level === 'medium' ? 'text-yellow-600 bg-yellow-50 border-yellow-200 dark:border-yellow-800'
    : summary.risk_level === 'high' ? 'text-red-600 bg-red-50 border-red-200 dark:border-red-800'
