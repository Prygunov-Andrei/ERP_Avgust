        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Дата согласования</th>
            ...
            {agreements.length > 0 ? (
              agreements.map((agreement) => (
                <tr key={agreement.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4"><span className="font-medium text-gray-900">{agreement.counterparty_detail.name}</span></td>
                  <td className="px-6 py-4"><span className="text-sm text-gray-600">{agreement.counterparty_detail.inn}</span></td>
                  <td className="px-6 py-4"><span className="text-sm text-gray-900">{formatDate(agreement.agreed_date)}</span></td>
                  <td className="px-6 py-4"><span className="text-sm text-gray-600">{agreement.notes || '—'}</span></td>
              ...
              <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500">Нет согласований</td></tr>
            )}
          </tbody>
        </table>
      </div>
